  ...
  const columns = React.useMemo(
    () => [
        ...
        cell: ({ row }) => (
          <div className="relative flex items-center">
            ...
            {user === 'publisher' || user === 'channel' ? (
              <>{row.status === 'pre_booked' ? <CircularTable /> : null}</>
            ) : null}
            {user === 'admin' ? (
              <>
                {row.status === 'open' ? (
      ...
      },
      {
        ...
        id: 'Формат',
        cell: ({ row }) => <FormatFormatter format={row.original.format} />,
        filterFn: 'includesString', //note: normal non-fuzzy filter column - case insensitive
      ...
      },
      {
        ...
        filterFn: 'includesString', //note: normal non-fuzzy filter column - case insensitive
        header: ({ table }) => {
          const hasOnlineViews = table
            .getPreFilteredRowModel()
            .rows.some((row) => row.original.online_views > 0)
